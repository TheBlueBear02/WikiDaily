...
export default function NotesGrid({ entries, isLoading, isError, error, onRetry }) {
  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div className="text-lg font-semibold tracking-tight text-primary">Your notes</div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-5">
          <div className="text-sm font-medium text-rose-900">Could not load your notes.</div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 text-center">
          <div className="text-sm font-medium text-primary">No notes yet.</div>
        ...
        <>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {visibleEntries.map((entry) => (
              <NoteCard
                key={`${entry?.updated_at ?? entry?.created_at ?? 'date'}-${entry?.wiki_slug ?? 'null'}`}
                entry={entry}
              />
            ))}
          </div>
